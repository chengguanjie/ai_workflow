...
import { nodeStyles, getTypeLabel, nodeTypes, ConditionNode, MediaDataImageNode, MediaDataAudioNode, MediaDataVideoNode, MediaDataDataNode } from './index'

// Helper to create complete NodeProps for testing
const createNodeProps = (id: string, type: string, data: Record<string, unknown>) => ({
  id,
  data,
  selected: false,
  type,
...
describe('Property 4: Node Style Consistency', () => {
  /**
   * For any registered node type, nodeStyles should contain icon, color, and bgColor
   */
  it('every node type should have complete style definition with icon, color, and bgColor', () => {
    fc.assert(
      fc.property(
        ...
        (nodeType) => {
          ...
          // Style should have bgColor property (a Tailwind class string)
  ...
  /**
   * Background color classes should follow Tailwind naming convention
   */
  it('bgColor classes should contain bg-{color} and border-{color} patterns', () => {
    fc.assert(
      fc.property(
        fc.constantFrom(...allNodeTypes),
        (nodeType) => {
          ...
          // bgColor should contain bg- class
          expect(style.bgColor).toContain('bg-')
          
          // bgColor should contain border- class
          expect(style.bgColor).toContain('border-')
        }
      ),
      { numRuns: 100 }
    )
...
describe('Property 13: Condition Node Dual Handles', () => {
  ...
  it('condition node should have two output handles with correct ids', () => {
    ...
    const handleIds = Array.from(sourceHandles).map(h => h.getAttribute('data-handleid'))
  ...
  it('condition node output handles should have distinct visual styles', () => {
    ...
    const trueHandle = Array.from(sourceHandles).find(h => h.getAttribute('data-handleid') === 'true')
    const falseHandle = Array.from(sourceHandles).find(h => h.getAttribute('data-handleid') === 'false')
...
describe('Property 14: Media/Data Node Mode Indicator', () => {
  ...
  const renderMediaDataNode = (nodeType: string, mode: 'input' | 'output' = 'input') => {
      ...
      fc.property(
        ...
        (nodeType) => {
          ...
          const { unmount } = render(
            <ReactFlowProvider>
              <Component
                {...createNodeProps(`test-${nodeType}-default`, nodeType, mockData)}
              />
            </ReactFlowProvider>
          )
