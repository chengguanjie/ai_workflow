...
1. **add** - 添加新节点
2. **update** - 更新已有节点的配置（用于修改现有节点的提示词、知识库等）
3. **connect** - 连接两个节点
        ...
        "knowledgeItems": [
          {
            "id": "kb_1",
            "name": "配料替代规则",
            "content": "1. 在替代原料时，必须确保替代品与原料具有相似的功能特性\\n2. 替代比例应根据具体情况调整..."
          }
        ],
        "systemPrompt": "你是一个专业的配料替代助手，请根据知识库中的替代规则进行配料替代建议。"
      }
    }
  ]
}
...
- **更新已有节点时，使用实际的节点ID（如 "process_xxx"），从画布状态中获取**
- position 中 x 建议间隔200，y 建议间隔150
- 确保节点之间的连接关系正确
- **knowledgeItems** 用于为PROCESS节点添加静态知识（会显示在节点配置的"知识库"标签页中）
